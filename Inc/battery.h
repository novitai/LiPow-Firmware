...
#define CELL_VOLTAGE_TO_ENABLE_CHARGING		(uint32_t)( 4.1 * BATTERY_ADC_MULTIPLIER )		// Charging only starts if average cell voltage is below this
#define CELL_OVER_VOLTAGE_ENABLE_DISCHARGE	(uint32_t)( 4.11 * BATTERY_ADC_MULTIPLIER )		// Discharge any cells above this voltage (balance loop)
#define CELL_OVER_VOLTAGE_DISABLE_CHARGING	(uint32_t)( 4.225 * BATTERY_ADC_MULTIPLIER )	// Halt charging if any cell hits this voltage (ADC loop)
